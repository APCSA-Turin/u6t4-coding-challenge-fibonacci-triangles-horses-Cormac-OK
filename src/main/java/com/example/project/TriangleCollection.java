...
public class TriangleCollection {
  ...
  public void shiftTriangles(int increment) {
    for (int t = 0; t < collection.length; t ++){
      for (int p = 0; p < collection[t].getVertices().length; p++){
          collection[t].getVertices()[p].setX(collection[t].getVertices()[p].getX()+increment);
          collection[t].getVertices()[p].setY(collection[t].getVertices()[p].getY()+increment);
      }
    }
  }
